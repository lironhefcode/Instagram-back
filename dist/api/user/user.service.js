...
async function handlefollow(username) {
    try {
        ...
        if (secondaryUser && loggedUser) {
            ...
            if (isFollowed) {
                secondaryUserFollower = secondaryUser.followers.filter(user => user._id !== mongodb_1.ObjectId.createFromHexString(secondaryUser._id));
                loggedUserFollowing = loggedUser.following.filter(user => user._id !== mongodb_1.ObjectId.createFromHexString(secondaryUser._id));
            }
            else {
            ...
            await collection.updateOne({ _id: mongodb_1.ObjectId.createFromHexString(secondaryUser._id) }, { $set: { followers: secondaryUserFollower } });
            const updatedUser = await collection.updateOne({ _id: mongodb_1.ObjectId.createFromHexString(loggedUser._id) }, { $set: { following: loggedUserFollowing } });
            return updatedUser;
        }
        else {
...
async function handleLike(storyId) {
    try {
        ...
        if (loggedUser) {
            ...
            const collection = await db_service_1.dbService.getCollection(collectionName);
            collection.updateOne({ _id: mongodb_1.ObjectId.createFromHexString(loggedUser._id) }, { $set: { likedStoryIds: likedStoryIds } });

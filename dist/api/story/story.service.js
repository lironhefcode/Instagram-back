...
async function getStoryById(storyId) {
  ...
  const story = await collection.findOne({
    _id: mongodb_1.ObjectId.createFromHexString(storyId),
  });
...
async function like(storyId) {
  try {
    ...
    const isLiked = story.likedBy.some(
      (user) =>
        user._id ===
        mongodb_1.ObjectId.createFromHexString(userFromCookeis._id),
    );
    let likedBy;
    if (isLiked) {
      ...
      const newLikeBy =
        user_service_1.userService.createminiUser(userFromCookeis);
